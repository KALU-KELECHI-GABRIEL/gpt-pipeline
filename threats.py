...
api_key = "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"
openai.api_key = api_key


# # Load the JSON file- uncomment this and comment the next block if you want to load the data from the file
# with open('./data_acm_final.json', 'r') as file_acm, open('./data_ieee_final.json', 'r') as file_ieee:
#     data_ieee = json.load(file_ieee)
#     data_acm = json.load(file_acm)

with open('./test.json', 'r') as file_acm, open('./test.json', 'r') as file_ieee:
    data_acm = json.load(file_acm)
    data_ieee = json.load(file_ieee)
...
for entry in data_acm:
    prompt_message = f"From the introduction of the paper provided, we want to determine 1. Threat type: what is the ml bug/vulnerability/attack raised in the paper that the paper wants to address; 2 Cause: What is the cause of ML-related problems in software that has been raised in the paper? This cause must involve an ML product, architecture, dataset, etc.  3. Causal Vector: Is the identified cause caused by software developers, data controllers/processors, organizations, or external parties? 4. Description: what is a summarized description of the threat?. Can you present this answer as a json format.\n\nIntroduction: {entry['Introduction']}"
    ...
    # print(verdict, 'me love you for a very long time')
...
with open('modified_data_ieee.json', 'w') as file:
    json.dump(data_ieee, file, indent=2)

with open('modified_data_acm.json', 'w') as file:
    json.dump(data_acm, file, indent=2)